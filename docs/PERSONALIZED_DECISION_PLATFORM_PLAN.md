...
Status: In progress on local post-submission branch `codex/personalized-decisions`

Current checkpoint, September 3, 2026: the shared profile and decision contracts, the thin gift proof at `/workspace`, the thin two-person date proof at `/date`, the thin vacation-package proof at `/vacation`, the unified decision agent at `/decide`, and the verified staffing vertical (Milestone 6) are implemented and tested locally. The unified agent exposes one `plan_decision` WebMCP tool, one bounded planning endpoint, and a separate no-store profile-proposal endpoint. It dispatches each explicit vertical (gift, date, vacation, staffing) through a fixed strategy registry, pins the evidence origin, returns one inspectable envelope, and supports linked full-context replacement without storing a decision. Professional service Offers, controlled trade and creative fixtures, credential and equipment requirement matching, quote-mode accounting, missing role detection, and human-only provider review boundaries are verified. The fuller Milestone 4 and Milestone 5 scopes, including distance evidence, accessibility evidence, date-specific inventory revalidation, item-level revision, broader supply, destination discovery, and account sync (Milestone 7), remain roadmap work. None of this post-submission work has been deployed or added to the live Devpost submission.
...
The next highest-leverage slice is verified staffing supply. Extend the same Offer protocol with provider identity, credential status and verification source, service area, availability windows, price basis, and portfolio evidence. Keep contact and contracting behind a separate human approval boundary. Account sync and cross-device memory remain later work and must not weaken the local-first consent model.
