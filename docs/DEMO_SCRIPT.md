...
Narration: "A commerce agent should receive explicit capabilities and source labels, not guess through visual controls. Agentic is a structured agent view over exact allowlisted product origins. This recording uses a controlled public catalog with original demo data."
...
Show `field-notebook`, `modular-desk-tray`, the `LIVE DEMO` adapter badge, and the activity entry.

Narration: "The default origin serves live HTTPS product JSON from original fixture content. The interface labels its controlled demo mode separately from adapter liveness and never presents it as merchant inventory."
...
Show the canonical URL as text, `LIVE PAGE MARKDOWN`, stripped Markdown, structured Offer projection, and field provenance. The controlled origin navigation and footer must be absent from the stripped projection.
