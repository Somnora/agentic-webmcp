...
The post-submission release candidate turns that evidence layer into one personalized decision agent. A person can start with a sparse goal and budget or add the context that makes a recommendation meaningful: a recipient's interests and existing items, what two people enjoy and have already done together, places previously visited, childhood or adult memories, desired novelty, or the roles, credentials, equipment, location, and schedule for a project. One `plan_decision` tool routes an explicit gift, date, vacation, or staffing request through a fixed deterministic strategy and returns one inspectable envelope.

Gift results filter hard exclusions and duplicates before ranking. Date results balance two people across low-cost, balanced, and special-occasion plans. Vacation results package lodging, transportation, dining, and activities into value, balanced, and signature tiers with category totals, contingency, and unknown costs. Staffing results assemble eligible controlled providers, surface missing roles or qualifications, and keep provider source opening behind a separate destination and transmitted-information review.
...
Origin content is always treated as untrusted and rendered safely. The application has no agent-side checkout, payment, Admin API, merchant account, booking, provider-contact, quote request, contract, cookies, analytics identifiers, or hidden approval path. Date and vacation outcomes may create a tentative memory proposal, but only a visible human approval can save the exact reviewed fact in that browser. Saved facts remain unselected until the person chooses them for a later decision.
...
The current local release candidate passes strict TypeScript checking, 214 automated tests across 28 test files, seven running-Worker release-candidate checks, and deployment dry runs for both Cloudflare Workers. A browser acceptance pass also exercised all four strategies through WebMCP, the complete outcome-memory consent cycle, eligible and ineligible staffing source review, the homepage entry point, and a 390-pixel mobile layout with no horizontal overflow or console errors. These checks do not mean the release candidate is public: it has not been pushed or deployed, and the live build must be verified again after explicit release approval.
