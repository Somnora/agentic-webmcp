...
The deadline-extension release turns that evidence layer into one personalized decision agent. A person can start with a sparse goal and budget or add the context that makes a recommendation meaningful: what they want to buy and already own, a recipient's interests and occasion, what two people enjoy and have already done together, places previously visited, childhood or adult memories, desired novelty, or the roles, credentials, equipment, location, and schedule for a project. One `plan_decision` tool routes an explicit shopping, gift, date, vacation, or staffing request through a fixed deterministic strategy and returns one inspectable envelope.

Shopping results compare source-backed product options for the user; gift results add recipient, occasion, and deadline context. Both filter hard exclusions and duplicates before ranking. Date results balance two people across low-cost, balanced, and special-occasion plans. Vacation results package lodging, transportation, dining, and activities into value, balanced, and signature tiers with category totals, contingency, and unknown costs. Staffing results assemble eligible controlled providers, surface missing roles or qualifications, and keep provider source opening behind a separate destination and transmitted-information review.
...
The release passes strict TypeScript checking, 217 automated tests across 28 test files, eight running-Worker release checks, and deployment dry runs for both Cloudflare Workers. Browser acceptance exercises all five strategies through WebMCP, the complete outcome-memory consent cycle, eligible and ineligible staffing source review, the homepage entry point, and a 390-pixel mobile layout with no horizontal overflow or console errors. The public `/health` response exposes the exact deployed commit and Worker version.
